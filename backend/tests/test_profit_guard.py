...
def _passing_gas(expected=D("0.20"), stress=D("0.30"), regime=GasRegime.NORMAL) -> GasAssessment:
    return GasAssessment(chain=Chain.SOLANA, regime=regime, current_gas_cost_usd=expected, expected_gas_cost_usd=expected, stress_gas_cost_usd=stress, gas_pct_of_gross=D("1"), passed=True)
...
def test_dex_attribution_does_not_double_count_router_fee(settings):
    ...
    buy = cex_quote("kraken", OrderSide.BUY, D("10"), D("100"), fee_pct=D("0.26"))
    sell = dex_quote("jupiter", OrderSide.SELL, D("10"), D("103"), D("102.5"), impact_pct=D("0.1"), fee_pct=D("0.30"))
    ...
    attributed = a.gross_profit - c.slippage - c.price_impact - c.dex_swap_fee - c.buy_trading_fee - c.sell_trading_fee
...
def test_elevated_gas_regime_raises_required_profit(settings):
    ...
    normal = ProfitGuard().assess(ProfitGuardInputs(buy=buy, sell=sell, settings=settings, gas=_passing_gas()))
    elevated = ProfitGuard().assess(ProfitGuardInputs(buy=buy, sell=sell, settings=settings, gas=_passing_gas(regime=GasRegime.ELEVATED)))
    assert elevated.required_min_profit == normal.required_min_profit * settings.gas.elevated_profit_multiplier


def test_extreme_gas_regime_blocks_on_chain(settings):
    buy = cex_quote("a", OrderSide.BUY, D("10"), D("100"), fee_pct=D("0.1"))
    sell = dex_quote("j", OrderSide.SELL, D("10"), D("110"), D("109.8"))
    a = ProfitGuard().assess(ProfitGuardInputs(buy=buy, sell=sell, settings=settings, gas=_passing_gas(regime=GasRegime.EXTREME)))
...
def test_experience_extra_buffer_reduces_expected(settings):
    ...
    learned = ProfitGuard().assess(ProfitGuardInputs(buy=buy, sell=sell, settings=settings, experience_extra_pct=D("0.10")))
...
def test_acceptance_1_headline_profit_but_true_profit_below_minimum(settings):
    ...
    buy = cex_quote("kraken", OrderSide.BUY, size, D("100"), avg_price=D("100.40"), fee_pct=D("0.25"))  # slippage $4, fee ~$2.5
    sell = dex_quote("jupiter", OrderSide.SELL, size, D("102"), D("101.60"), impact_pct=D("0.20"), fee_pct=D("0.20"))  # dex fee ~$2, impact $2
...
def test_acceptance_2_expected_positive_but_worst_case_negative(settings):
    ...
    size = D("10")
    buy = cex_quote("kraken", OrderSide.BUY, size, D("100"), avg_price=D("100.90"), fee_pct=D("0.30"))  # slippage $9
    sell = cex_quote("coinbase", OrderSide.SELL, size, D("105"), avg_price=D("104.10"), fee_pct=D("0.30"))  # slippage $9
...
def test_acceptance_3_flash_loan_eligible_when_everything_passes(settings):
    ...
    size = D("1")
    buy = dex_quote("uniswap-base", OrderSide.BUY, size, D("2000"), D("2001"), impact_pct=D("0.03"), fee_pct=D("0.05"), chain=Chain.BASE, symbol="ETH/USDC")
    sell = dex_quote("pancakeswap-base", OrderSide.SELL, size, D("2030"), D("2028.7"), impact_pct=D("0.03"), fee_pct=D("0.05"), chain=Chain.BASE, symbol="ETH/USDC")
    gas = _passing_gas(expected=D("1.00"), stress=D("1.50"))
    fl = FlashLoanTerms(provider="aave_v3", loan_amount_usd=buy.quote_amount, fee_pct=D("0.05"), simulation_passed=True, atomic=True)
...
def test_acceptance_3b_flash_loan_blocked_without_simulation(settings):
    ...
    fl = FlashLoanTerms(provider="aave_v3", loan_amount_usd=buy.quote_amount, fee_pct=D("0.05"), simulation_passed=None, atomic=True)
    a = ProfitGuard().assess(ProfitGuardInputs(buy=buy, sell=sell, settings=settings, gas=_passing_gas(), flash_loan=fl))
...
def test_gas_guard_blocks_when_gas_dominates_gross(settings):
    ...
    snap = gas_snapshot(Chain.BASE, gas_price=D("30"), native_usd=D("3000"))  # 30 gwei * 200k gas = 0.006 ETH = $18
...
def test_gas_becomes_expensive_before_execution_blocks(settings):
    ...
    sell = dex_quote("uniswap-base", OrderSide.SELL, D("2"), D("3030"), D("3028"), chain=Chain.BASE, symbol="ETH/USDC")
    guard = GasGuard(settings.gas)
...
def test_carry_round_trips_double_fees_and_halve_worst_case_funding_income(settings):
    ...
    sell = cex_quote("binance-perp", OrderSide.SELL, D("1"), D("60030"), fee_pct=D("0.05"), symbol="BTC/USDT:USDT")
    one = ProfitGuard().assess(ProfitGuardInputs(buy=buy, sell=sell, settings=settings, funding_cost_usd=D("-40")))
    assert one.expected_costs.funding == D("-40") and one.worst_case_costs.funding == D("-20")
    two = ProfitGuard().assess(ProfitGuardInputs(buy=buy, sell=sell, settings=settings, funding_cost_usd=D("-40"), round_trips=2))
