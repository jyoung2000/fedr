...
async def get_settings(app=Depends(require_app)):
    return {"settings": app.settings.to_public_dict(), "strategies_catalog": [{"key": i.key.value, "name": i.name, "description": i.description, "default_on": i.default_on, "risks": i.risks, "carry": i.carry, "atomic": i.atomic} for i in CATALOG.values()], "env": {"live_trading_allowed": app.env.live_trading_allowed, "auth_enabled": bool(app.env.auth_token), "gateway_enabled": app.env.gateway_enabled, "gateway_url": app.env.gateway_url, "private_rpc": bool(app.env.evm_private_rpc), "rpc": {c: bool(app.env.rpc_for(c)) for c in ("ethereum", "base", "arbitrum", "optimism", "polygon", "bsc", "avalanche", "solana")}, "flashloan_contracts": {c: bool(app.env.flashloan_contract_for(c)) for c in ("ethereum", "base", "arbitrum", "optimism", "polygon")}, "telemetry": app.env.telemetry_enabled, "data_dir": str(app.env.data_dir)}}
...
async def put_settings(body: Patch, app=Depends(require_app)):
    if "live" in body.patch and any(k not in ("paper_completed", "shadow_reviewed") for k in body.patch["live"]):
