...
async def opportunities(app=Depends(require_app), limit: int = 100):
    ...
    return {"items": [{"id": r.id, "strategy": r.strategy, "pair": r.pair, "route": r.route, "decision": r.decision, "gross_pct": r.gross_pct, "expected_net_usd": r.expected_net_usd, "worst_case_usd": r.worst_case_usd, "risk_score": r.risk_score, "created_at": r.created_at.isoformat(), "explanation": r.payload.get("explanation"), "block_reasons": r.payload.get("block_reasons", [])} for r in rows]}
...
async def audit(app=Depends(require_app), limit: int = 100, event_type: str | None = None):
    ...
    return {"items": [{"id": r.id, "ts": r.ts.isoformat(), "mode": r.mode, "event_type": r.event_type, "actor": r.actor, "summary": r.summary, "payload": r.payload} for r in rows]}
...
async def risk_events(app=Depends(require_app), limit: int = 50):
    ...
    return {"items": [{"id": r.id, "ts": r.ts.isoformat(), "severity": r.severity, "kind": r.kind, "detail": r.detail, "payload": r.payload} for r in rows]}
...
async def backtests(app=Depends(require_app)):
    ...
    return {"items": [{"id": r.id, "ts": r.ts.isoformat(), "label": r.label, "params": r.params, "results": r.results} for r in rows]}
