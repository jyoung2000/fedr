...
async def run_backtest(
    ...
    *,
    ...
    ctx = EngineContext(
        ...
        repo=None,
        hub=hub,
        gas_oracle=GasOracle(hub.prices.get),
        breakers=CircuitBreakerManager(),
        inventory=inventory,
        experience=ExperienceEngine(None, enabled=False),
        rebalancer=Rebalancer(s.rebalance, inventory),
    )
    ...
    for t in range(ticks):
        ...
        for venue in ledger.venues():
            inventory.update_balances(
                venue, ledger.balances(venue), kind="chain" if venue in {c.value for c in Chain} else "cex"
            )
            ...
            stats[
                {
                    ...
                    TradeStatus.HEDGED: "hedged",
                    TradeStatus.ABORTED: "aborted",
                }.get(tr.status, "failed")
            ] += 1
    ...
    return {
        ...
        "params": {
            ...
            "min_profit_usd": str(s.trading.min_profit_usd),
            "stress_multiplier": str(s.paper.stress_multiplier),
        },
        "results": {
            **{k: (str(v) if isinstance(v, Decimal) else v) for k, v in stats.items()},
            "start_value_usd": str(start_value),
            "end_value_usd": str(end_value),
            "prediction_error_total": str(stats["estimated_net"] - stats["net"]),
        },
    }
    ...
    with open(path) as fh:
        for line in fh:
            ...
            ticks.append(
                [
                    OrderBook.from_raw(
                        b["venue"], b["symbol"], b["bids"], b["asks"], ts_ms=b.get("ts_ms"), source="replay"
                    )
            ...
            "books": [
                {
                    ...
                    "symbol": b.symbol,
                    "ts_ms": b.ts_ms,
                    "bids": [[str(l.price), str(l.amount)] for l in b.bids[:25]],
                    "asks": [[str(l.price), str(l.amount)] for l in b.asks[:25]],
                }
                for b in books
            ]
        }
