...
class ShadowRecorder:
    ...
    async def record(self, opp: Opportunity) -> None:
        ...
        rec = ShadowRecord(
            ...
            payload={
                "costs": opp.profit.expected_costs.as_dict(),
                "risk": opp.risk.score,
                "buy_price": str(opp.buy.avg_price),
                "sell_price": str(opp.sell.avg_price),
            },
        )
        if self.ctx.repo:
    ...
    async def _reprice(self, rec: ShadowRecord, opp: Opportunity) -> None:
        ...
        try:
            ...
            if b is None or s is None:
                return
            bq, sq = await asyncio.gather(
                b.get_quote(
                    opp.pair, OrderSide.BUY, opp.size_base, order_book=self.ctx.hub.get_book(b.name, opp.pair)
                ),
                s.get_quote(
                    opp.sell.symbol,
                    OrderSide.SELL,
                    opp.size_base,
                    order_book=self.ctx.hub.get_book(s.name, opp.sell.symbol),
                ),
            )
            costs = opp.profit.expected_costs
            non_exec = costs.total() - (
                costs.slippage
                + costs.price_impact
                + costs.dex_swap_fee
                + costs.buy_trading_fee
                + costs.sell_trading_fee
            )
            ...
            rec.payload = {
                **rec.payload,
                "reprice_buy": str(bq.avg_price),
                "reprice_sell": str(sq.avg_price),
                "reprice_delay_s": self.reprice_delay_s,
            }
            if self.ctx.repo:
                await self.ctx.repo.add_shadow(rec)
