...
CONTRACT_ABI: list[dict[str, Any]] = [
        ...
        "inputs": [
            ...
            {"name": "amount", "type": "uint256"},
            {
                ...
                "components": [
                    ...
                    {"name": "legA", "type": "tuple", "components": [{"name": "router", "type": "address"}, {"name": "kind", "type": "uint8"}, {"name": "fee", "type": "uint24"}, {"name": "minAmountOut", "type": "uint256"}]},
                    {"name": "legB", "type": "tuple", "components": [{"name": "router", "type": "address"}, {"name": "kind", "type": "uint8"}, {"name": "fee", "type": "uint24"}, {"name": "minAmountOut", "type": "uint256"}]},
    ...
    {"type": "function", "name": "allowedRouters", "stateMutability": "view", "inputs": [{"type": "address"}], "outputs": [{"type": "bool"}]},
]
...
class FlashLoanEngine:
    ...
    def available_on(self, chain: Chain) -> tuple[bool, str]:
        ...
        if not self.env.flashloan_contract_for(chain.value):
            return False, f"no FlashLoanArbitrage contract configured for {chain.value} (FEDR_FLASHLOAN_CONTRACT_{chain.value.upper()})"
    ...
    async def simulate(self, cand, buy_q, sell_q, size: Decimal) -> bool:
        ...
        if self.ctx.mode in (TradingMode.SIMULATION, TradingMode.PAPER):
            ...
            res = SimulationResult(profit >= min_profit_quote, "" if profit >= min_profit_quote else f"would revert: net {fmt_money(profit)} below minProfit {fmt_money(min_profit_quote)}", gas_estimate=420_000, expected_profit_asset=profit)
    ...
    async def _simulate_onchain(self, chain: Chain, cand, buy_q, sell_q, size: Decimal) -> SimulationResult:
        ...
        if params is None:
            return SimulationResult(False, "route cannot be encoded: DEX venues must expose router/token addresses via Gateway token metadata")
    ...
    def _build_params(self, cand, buy_q, sell_q):
        ...
        p = (quote_addr, (router_a, 0, int(getattr(cand.buy.spec, "fee_tier", 3000)), min_a), (router_b, 0, int(getattr(cand.sell.spec, "fee_tier", 3000)), min_b), min_profit, deadline)
    ...
    async def execute(self, opp: Opportunity, opportunity_engine, trigger: str = "auto") -> TradeRecord:
        ctx = self.ctx
        s = ctx.settings
        tr = TradeRecord(id=new_id("fltrd"), mode=ctx.mode, strategy=Strategy.FLASH_LOAN, opportunity_id=opp.id, pair=opp.pair, route=opp.route, size_base=opp.size_base, status=TradeStatus.PENDING, estimated_gross=opp.profit.gross_profit, estimated_net=opp.profit.expected_net_profit, estimated_worst_case=opp.profit.worst_case_profit, estimated_costs=opp.profit.expected_costs.as_dict())
        ...
        if fresh is None or fresh.decision is not Decision.SAFE_TO_EXECUTE:
            ...
            return await self._abort(tr, "route no longer profitable after quote refresh: " + "; ".join(reasons))
        ...
        try:
            ...
            if ctx.repo:
                ...
                if tr.actual_net is not None and tr.status is TradeStatus.FILLED:
                    await ctx.repo.add_pnl(ctx.mode, gross=outcome.actual_gross, trading_fees=outcome.trading_fees, gas=outcome.gas, net=outcome.actual_net, win=outcome.actual_net > 0)
                await ctx.repo.audit(ctx.mode, "execution", tr.explanation, {"trade_id": tr.id, "status": tr.status.value})
    ...
    async def _settle_paper(self, tr: TradeRecord, fresh: Opportunity) -> None:
        ...
        buy_req = OrderRequest(venue=fresh.buy.venue, symbol=fresh.pair, side=OrderSide.BUY, amount=fresh.size_base, limit_price=fresh.buy.avg_price, extra={"quoted_price": str(fresh.buy.avg_price), "flash_loan": True})
        sell_req = OrderRequest(venue=fresh.sell.venue, symbol=fresh.pair, side=OrderSide.SELL, amount=fresh.size_base, limit_price=fresh.sell.avg_price, extra={"quoted_price": str(fresh.sell.avg_price), "flash_loan": True})
        tr.buy = OrderResult(request=buy_req, order_id=new_id("fl"), status=OrderStatus.FILLED, filled=fresh.size_base, avg_price=fresh.buy.avg_price, fee_quote=fee, gas_cost_usd=gas_usd, tx_hash=f"paper-flash-{tr.id}")
        tr.buy.fills.append(Fill(order_id=tr.buy.order_id, venue=fresh.buy.venue, symbol=fresh.pair, side=OrderSide.BUY, amount=fresh.size_base, price=fresh.buy.avg_price, fee_amount=fee, fee_asset=quote, gas_cost_usd=gas_usd))
        tr.sell = OrderResult(request=sell_req, order_id=new_id("fl"), status=OrderStatus.FILLED, filled=fresh.size_base, avg_price=fresh.sell.avg_price, tx_hash=f"paper-flash-{tr.id}")
        tr.sell.fills.append(Fill(order_id=tr.sell.order_id, venue=fresh.sell.venue, symbol=fresh.pair, side=OrderSide.SELL, amount=fresh.size_base, price=fresh.sell.avg_price, fee_amount=ZERO, fee_asset=quote))
    ...
    async def _broadcast(self, tr: TradeRecord, fresh: Opportunity, cand) -> None:
        ...
        s = self.ctx.settings
        rpc = self.env.rpc_for(chain.value)
        private_rpc = self.env.evm_private_rpc if (s.mev.enabled and s.mev.evm_private_submission and chain is Chain.ETHEREUM) else None
        wallet = self.wallets.bot_wallet("evm")
        if wallet is None:
        ...
        w3 = AsyncWeb3(AsyncHTTPProvider(rpc))
        contract = w3.eth.contract(address=w3.to_checksum_address(self.env.flashloan_contract_for(chain.value)), abi=CONTRACT_ABI)
        ...
        priority = min(int(D(s.gas.max_priority_fee_gwei) * Decimal(10**9)), int(fee_hist["reward"][-1][0]) if fee_hist.get("reward") else int(1e9))
        ...
        if Decimal(max_fee) / Decimal(10**9) > s.gas.max_gas_price_gwei:
            ...
            return
        tx = await fn.build_transaction({"from": acct.address, "nonce": await w3.eth.get_transaction_count(acct.address), "gas": int(gas * 1.15), "maxFeePerGas": max_fee, "maxPriorityFeePerGas": priority, "chainId": await w3.eth.chain_id})
        ...
        if int(receipt["status"]) != 1:
            ...
            tr.buy = OrderResult(request=buy_req, order_id=new_id("fl"), status=OrderStatus.FAILED, gas_cost_usd=gas_usd, tx_hash=tx_hash.hex(), error="reverted")
            tr.sell = OrderResult(request=sell_req, order_id=new_id("fl"), status=OrderStatus.FAILED, tx_hash=tx_hash.hex(), error="reverted")
        ...
        tr.buy = OrderResult(request=buy_req, order_id=new_id("fl"), status=OrderStatus.FILLED, filled=fresh.size_base, avg_price=fresh.buy.avg_price, fee_quote=fresh.buy.quote_amount * Decimal("0.0005"), gas_cost_usd=gas_usd, tx_hash=tx_hash.hex())
        tr.sell = OrderResult(request=sell_req, order_id=new_id("fl"), status=OrderStatus.FILLED, filled=fresh.size_base, avg_price=fresh.sell.avg_price, tx_hash=tx_hash.hex())
        tr.status = TradeStatus.FILLED
