...
class OpportunityEngine:
    ...
    # ------------------------------------------------------------------ sizing
    def pick_size(
        self, cand: RouteCandidate, price_usd: Decimal, portfolio: PortfolioState
        ...
        target_usd = (
            min(
                s.trading.max_trade_size_usd,
                portfolio.usable_capital_usd * s.risk.max_capital_usage_pct / HUNDRED,
            )
            if not cand.flash_loan
            else min(s.flash_loan.max_loan_usd, s.risk.max_flash_loan_usd)
        )
        if target_usd <= 0:
        ...
        for venue, side in ((cand.buy, OrderSide.BUY), (cand.sell, OrderSide.SELL)):
            if venue.kind in (VenueKind.CEX, VenueKind.PERP):
                book = self.ctx.hub.get_book(
                    venue.name, cand.pair if venue.kind is VenueKind.CEX else f"{base}/{quote}:{quote}"
                )
                if book is not None:
                    limit = self.ctx.slippage_guard.limit_for(
                        base, venue.name, f"{cand.buy.name}->{cand.sell.name}", target_usd
                    )
                    cap = book.max_size_within_slippage(side, limit)
                    if cap < size:
                        notes.append(
                            f"size capped by {venue.name} depth ({cap:.6f} {base} within {limit}% slippage)"
                        )
                        size = cap
    ...
    async def evaluate(self, cand: RouteCandidate, size: Decimal | None = None) -> Opportunity | None:
        ...
        if size is None and cand.flash_loan:
            ...
            for frac in self.FLASH_LOAN_SIZE_LADDER:
                ...
                if o is None:
                    continue
                if best is None or (o.decision is Decision.SAFE_TO_EXECUTE, o.profit.worst_case_profit) > (
                    best.decision is Decision.SAFE_TO_EXECUTE,
                    best.profit.worst_case_profit,
                ):
                    best = o
            if best is not None:
        ...
        if size <= 0 or size * px < s.trading.min_trade_size_usd:
            reasons.append(
                f"tradeable size {fmt_money(size * px)} below minimum {fmt_money(s.trading.min_trade_size_usd)}"
            )
            size = max(size, floor_to_step(s.trading.min_trade_size_usd / px, Decimal("0.000001")))
        # ---- quotes (concurrently) ----
        try:
            if info.carry:
                buy_q, sell_q, carry = await evaluate_carry(
                    ctx, cand.strategy, cand.buy, cand.sell, cand.pair, size
                )
            else:
                carry = None
                buy_q, sell_q = await asyncio.gather(
                    self._leg_quote(cand.buy, cand.pair, OrderSide.BUY, size),
                    self._leg_quote(cand.sell, cand.pair, OrderSide.SELL, size),
                )
    ...
    async def _leg_quote(
        self, venue: VenueConnector, pair: str, side: OrderSide, size: Decimal
    ) -> ExecutionQuote | None:
        book = (
            self.ctx.hub.get_book(venue.name, pair) if venue.kind in (VenueKind.CEX, VenueKind.PERP) else None
        )
    ...
    async def _assess(
        ...
        cand: RouteCandidate,
        info: StrategyInfo,
        buy_q: ExecutionQuote,
        sell_q: ExecutionQuote,
        size: Decimal,
        px: Decimal,
        portfolio: PortfolioState,
        pre_reasons: list[str],
        carry: CarryQuote | None,
    ) -> Opportunity:
        ctx = self.ctx
        s = ctx.settings
        base, quote = cand.pair.split("/")
        ...
        if cand.flash_loan and buy_q.chain is not None:
            ...
            gas = ctx.gas_guard.assess(
                snap, FLASH_LOAN_GAS_LIMIT, gross_ref, max_gas_override_usd=s.flash_loan.max_gas_usd, legs=1
            )
        elif on_chain_legs:
            assessments = []
        ...
        if cand.flash_loan:
            ...
            if ctx.flash_loan_simulator is not None:
                try:
                    ...
                    sim_ok = False
            fl_terms = FlashLoanTerms(
                provider=provider,
                loan_amount_usd=buy_q.quote_amount,
                fee_pct=FLASH_LOAN_FEE_PCT.get(provider, Decimal("0.09")),
                simulation_passed=sim_ok,
                atomic=buy_q.chain is not None and buy_q.chain is sell_q.chain,
            )
        # ---- profit guard ----
        ...
        explanation = (
            ...
            else "Blocked because " + "; ".join(reasons[:3]) + "."
        )
        opp = Opportunity(
            id=new_id("opp"),
            mode=ctx.mode,
            ...
            extra={
                ...
                "flash_loan": bool(cand.flash_loan),
                "carry": to_jsonable(carry) if carry else None,
                "experience_extra_pct": str(ctx.experience.extra_buffer_pct(rk)),
            },
        )
        return opp
    ...
    def _annotate_alternatives(self, opps: list[Opportunity]) -> None:
        ...
        for group in by_route.values():
            ...
            best = max(
                group,
                key=lambda o: (
                    o.decision is Decision.SAFE_TO_EXECUTE,
                    o.profit.worst_case_profit,
                    o.profit.expected_net_profit,
                ),
            )
            for o in group:
                o.alternatives = [
                    {
                        ...
                        "expected_net": str(a.profit.expected_net_profit),
                        "worst_case": str(a.profit.worst_case_profit),
                ...
                if o is not best:
                    o.block_reasons.append(
                        f"{best.strategy.value} route preferred: stronger worst-case economics ({fmt_money(best.profit.worst_case_profit)} vs {fmt_money(o.profit.worst_case_profit)})"
                    )
    ...
    def ranked(self) -> list[Opportunity]:
        ...
        return sorted(
            live,
            key=lambda o: (
                o.decision is not Decision.SAFE_TO_EXECUTE,
                -o.profit.worst_case_profit,
                -o.profit.expected_net_profit,
                o.risk.score,
            ),
            ...
            try:
                ...
                await repo.audit(
                    ...
                    o.explanation,
                    {
                        ...
                        "buy_price": str(o.buy.avg_price),
                        "sell_price": str(o.sell.avg_price),
                        "market_ts": {"buy": o.buy.market_ts_ms, "sell": o.sell.market_ts_ms},
                        "costs": o.profit.expected_costs.as_dict(),
                        "worst_case_costs": o.profit.worst_case_costs.as_dict(),
                        "gross": str(o.profit.gross_profit),
                        "expected_net": str(o.profit.expected_net_profit),
                        "worst_case": str(o.profit.worst_case_profit),
                        "risk_score": o.risk.score,
                        "decision": o.decision.value,
                        "reasons": o.block_reasons,
                    },
                )
            except Exception as exc:  # pragma: no cover
