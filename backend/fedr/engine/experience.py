...
def size_bucket(notional_usd: Decimal) -> str:
    for edge, name in ((Decimal("100"), "xs"), (Decimal("500"), "s"), (Decimal("2000"), "m"), (Decimal("10000"), "l")):
        if notional_usd <= edge:
            return name
    return "xl"


def route_key(mode: TradingMode, strategy: Strategy, buy_venue: str, sell_venue: str, pair: str, notional_usd: Decimal) -> str:
    return f"{mode.value}|{strategy.value}|{buy_venue}|{sell_venue}|{pair}|{size_bucket(notional_usd)}"


class ExperienceEngine:
    def __init__(self, repo: Repo | None, max_extra_buffer_pct: Decimal = Decimal("0.25"), enabled: bool = True):
        self.repo = repo
    ...
    async def record(
        ...
        *,
        ...
        if s.samples == 0:
            s.error_pct_ewma, s.slippage_bias_pct, s.fee_variance_pct, s.gas_variance_pct = map(str, (err_pct, slip_pct, fee_pct, gas_pct))
            ...
            s.fill_reliability = str(D(s.fill_reliability) * (1 - ALPHA) + (Decimal(1) if filled else ZERO) * ALPHA)
    ...
    def summary(self) -> list[dict]:
        return [
            {"key": s.key, "samples": s.samples, "error_pct": s.error_pct_ewma, "slippage_bias_pct": s.slippage_bias_pct, "fee_variance_pct": s.fee_variance_pct, "gas_variance_pct": s.gas_variance_pct, "fill_reliability": s.fill_reliability, "latency_ms": s.latency_ms_ewma, "extra_buffer_pct": s.extra_buffer_pct}
