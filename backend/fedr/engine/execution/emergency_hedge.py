...
class EmergencyHedger:
    ...
    async def hedge(self, trade: TradeRecord, pair: str, exposure_base: Decimal, exclude: set[str] | None = None) -> OrderResult | None:
        ...
        for c in self.ctx.connectors.values():
            ...
            score += 25 if c.kind is VenueKind.DEX else 0  # certainty: CEX fills are more predictable than tx inclusion
        ...
        for _, venue, q in candidates[:3]:
            ...
            req = OrderRequest(venue=venue.name, symbol=pair, side=side, amount=amount, limit_price=limit, time_in_force="IOC", client_order_id=new_id("hedge"), min_received=(q.quote_amount * (1 - tol)) if side is OrderSide.SELL else None, quote_id=q.quote_id, deadline_ms=self.ctx.settings.trading.execution_timeout_ms, extra={"quoted_price": str(q.avg_price), "emergency": True})
            ...
            try:
                if self.ctx.is_simulated_execution and self.ctx.paper_executor is not None:
                    ...
                    if side is OrderSide.SELL:
                        ...
                        await self.ctx.ledger.reserve(lv, quote, q.quote_amount * (1 + tol) * (1 + D(q.fee_pct or 0) / HUNDRED))
                    res = await self.ctx.paper_executor.execute_leg(venue, req, q)
                    if res.filled < amount:
                        rem = amount - res.filled
                        await self.ctx.ledger.release(lv, base if side is OrderSide.SELL else quote, rem if side is OrderSide.SELL else rem * q.avg_price)
                else:
