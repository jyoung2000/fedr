...
class ReconciliationReport:
    ...
    def as_dict(self) -> dict:
        return {
            "ok": self.ok,
            "checked_venues": self.checked_venues,
    ...
    async def run_once(self) -> ReconciliationReport:
        ...
        if not rep.ok:
            await ctx.breakers.trip(
                CircuitBreakerReason.BALANCE_DISCREPANCY, "; ".join(rep.discrepancies)[:250]
            )
            if ctx.repo:
                await ctx.repo.add_risk_event(
                    ctx.mode, "critical", "reconciliation", "; ".join(rep.discrepancies), rep.as_dict()
                )
                await ctx.repo.audit(
                    ctx.mode,
                    "reconciliation",
                    "Reconciliation found discrepancies - trading paused",
                    rep.as_dict(),
                )
        return rep
        ...
        for venue, assets in ctx.ledger.dump().items():
            ...
            for asset, (free, used) in assets.items():
                ...
                if used > 0 and not ctx.open_trade_ids:
                    ...
                    log.warning(
                        "released stale paper reservation", venue=venue, asset=asset, amount=str(used)
                    )
        if ctx.repo:
    ...
    async def _reconcile_real(self, rep: ReconciliationReport) -> None:
        ...
        for c in ctx.connectors.values():
            ...
            for asset, b in balances.items():
                ...
                if diff * px > BALANCE_TOLERANCE_ABS_USD and (
                    exp == 0 or diff / exp * 100 > BALANCE_TOLERANCE_PCT
                ):
                    rep.discrepancies.append(
                        f"{c.name}: {asset} expected {exp} but exchange reports {b.total}"
                    )
                self.expected[(lv, asset)] = (
                    b.total
                )  # after reporting, adopt reality so we don't repeat forever
        ...
        for tr in ctx.extra.get("recent_trades", []):
            for leg in (tr.buy, tr.sell, tr.hedge):
                if leg is not None and leg.status is OrderStatus.OPEN and leg.tx_hash:
                    ...
                    rep.discrepancies.append(
                        f"{leg.request.venue}: transaction {leg.tx_hash[:12]}… still unconfirmed"
                    )
