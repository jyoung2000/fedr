...
class DepositMonitor:
    ...
    def observe(
        self, chain: str, balances: dict[str, Decimal], expected_deltas: dict[str, Decimal] | None = None
    ) -> list[DepositEvent]:
    ...
    async def quote(self, chain: Chain, asset: str, amount: Decimal, destination: str) -> WithdrawalQuote:
        ...
        if asset != chain.native_token:
            return WithdrawalQuote(
                ...
                amount,
                False,
                "token withdrawals are not implemented in this build - withdraw native assets or use the exchange/DEX to convert first",
            )
        ...
        return WithdrawalQuote(
            chain.value, asset, amount, destination, fee_native, fee_native * px if px else None, amount, True
        )

    async def execute(
        self, chain: Chain, wallet_id: str, amount: Decimal, destination: str
    ) -> dict[str, Any]:
        rpc = self.rpc_for(chain)
        if not rpc:
