...
class FedrApp:
    ...
    async def start(self) -> None:
        ...
        if self.settings.general.mode is TradingMode.LIVE and not (self.settings.live.activated and self.env.live_trading_allowed):
        ...
        self.ctx.breakers.restore([Breaker(reason=CircuitBreakerReason(r.reason), detail=r.detail, scope=r.scope, auto=r.auto, tripped_at_ms=int(r.tripped_at.timestamp() * 1000)) for r in rows])
        ...
        log.info("startup", status=self.status_message, mode=self.settings.general.mode.value, port=self.env.port)
        await self.repo.audit(self.mode, "system", f"Startup: {self.status_message}", self.startup_check.as_dict())
    ...
    async def _build_context(self) -> None:
        ...
        experience = ExperienceEngine(self.repo, self.settings.advanced.experience_max_extra_buffer_pct, self.settings.advanced.experience_learning_enabled)
        ...
        self.ctx = EngineContext(env=self.env, settings=self.settings, repo=self.repo, hub=hub, gas_oracle=gas_oracle, breakers=breakers, inventory=inventory, experience=experience, rebalancer=rebalancer)
        ...
        async with self._rebuild_lock:
            ...
            if mode is TradingMode.SIMULATION:
                ...
                for name, chain in (("jupiter", Chain.SOLANA), ("uniswap-base", Chain.BASE), ("uniswap-arbitrum", Chain.ARBITRUM)):
                    ctx.connectors[name] = SyntheticDexVenue(self.synthetic, name, dex_pairs_for(chain, pairs), chain)
                ctx.connectors["binance-perp"] = SyntheticPerpVenue(self.synthetic, "binance-perp", pairs, Decimal("0.05"))
            else:
            ...
            if mode in (TradingMode.SIMULATION, TradingMode.PAPER):
                ...
                ctx.paper_executor = PaperExecutor(ctx.ledger, lambda: self.settings.paper, ctx.gas_oracle.snapshot, ctx.gas_guard, rng=random.Random(self.env.demo_seed))
            await self._refresh_balances()
            for chain in self._chains_in_use():
                await ctx.gas_oracle.refresh(chain)
            await ctx.hub.start([c for c in ctx.connectors.values() if c.connected], lambda c: [p for p in pairs if c.supports_symbol(p)] + ([f"{p.split('/')[0]}/{p.split('/')[1]}:{p.split('/')[1]}" for p in pairs] if c.kind is VenueKind.PERP else []))
            if mode is TradingMode.SIMULATION:
            ...
            log.info("connectors rebuilt", mode=mode.value, venues=[c.name for c in ctx.connectors.values() if c.connected])
    ...
    async def _build_cex_connectors(self, mode: TradingMode, pairs: list[str]) -> None:
        ...
        for exchange_id, creds in wanted.items():
            ...
            if sandbox and not spec.sandbox:
                ...
                continue
            c = CcxtConnector(exchange_id, mode, creds, sandbox=sandbox, timeout_s=s.advanced.connector_timeout_s)
            ...
            if spec.perps and (s.strategies.spot_perp or s.strategies.funding or s.strategies.basis):
                ...
                pc = CcxtConnector(pid, mode, creds, sandbox=sandbox, market_type="swap", name=f"{exchange_id}-perp", timeout_s=s.advanced.connector_timeout_s)
    ...
    async def _build_dex_connectors(self, mode: TradingMode, pairs: list[str]) -> None:
        ...
        for name in enabled:
            ...
            address = self.wallets.address_for_chain(spec.chain, wallet_mode) if mode in (TradingMode.TESTNET, TradingMode.LIVE) else None
            c = GatewayConnector(spec, self.gateway, mode, address, pairs, s.slippage.dex_slippage_tolerance_pct, native_usd_price=ctx.hub.prices.get, testnet=testnet)
        ...
        while True:
            try:
                ...
                if self.settings.general.shadow_mode and self.shadow:
                    ...
                    continue
                if not self.settings.general.bot_enabled or not self.settings.trading.auto_execute or self.ctx.emergency_stop:
                    continue
                for o in opps:
                ...
                for chain in self._chains_in_use():
                    ...
                    if snap is not None:
                        ...
                        if regime.value == "extreme":
                            await self.ctx.breakers.trip(CircuitBreakerReason.GAS_SPIKE, f"gas on {chain.value} is {snap.gas_price_native} (extreme regime)", scope=f"chain:{chain.value}")
                        else:
                            await self.ctx.breakers.reset(CircuitBreakerReason.GAS_SPIKE, scope=f"chain:{chain.value}")
            except asyncio.CancelledError:
                raise
            except Exception as exc:
        ...
        for c in list(ctx.connectors.values()):
            ...
            if c.kind is VenueKind.DEX:
                ...
                for ev in self.deposits.observe(lv, {a: b.total for a, b in bals.items()}):
                    self._push_event("deposit", {"chain": ev.chain, "asset": ev.asset, "amount": str(ev.amount)})
                    if self.repo:
                        await self.repo.audit(self.mode, "wallet", f"Deposit detected: {ev.amount} {ev.asset} on {ev.chain}", {"chain": ev.chain, "asset": ev.asset, "amount": str(ev.amount)})
            ...
            if self.repo:
                await self.repo.snapshot_balances(self.mode, lv, {a: (b.free, b.used, (b.total * px) if (px := ctx.price_usd(a)) else None) for a, b in bals.items()})
    ...
    async def _load_daily_stats(self) -> None:
        ...
        self.ctx.failed_trades_last_hour = await self.repo.count_failed_trades_since(self.mode, utcnow() - timedelta(hours=1))
    ...
    async def _on_trade(self, tr: TradeRecord) -> None:
        self._push_event("trade", {"id": tr.id, "status": tr.status.value, "pair": tr.pair, "route": tr.route, "estimated_net": str(tr.estimated_net), "actual_net": str(tr.actual_net) if tr.actual_net is not None else None, "explanation": tr.explanation})
    ...
    async def update_settings(self, patch: dict[str, Any], actor: str = "user") -> AppSettings:
        ...
        if new.general.mode is TradingMode.LIVE and not (self.settings.live.activated and self.env.live_trading_allowed):
        ...
        if self.repo:
            ...
            await self.repo.audit(new.general.mode, "mode_change" if mode_changed else "settings_change", f"Settings updated ({', '.join(patch.keys())})" if not mode_changed else f"Mode changed {old_mode.value} -> {new.general.mode.value}", {"patch": _redact(patch)}, actor=actor)
        if structural:
            await self.rebuild_connectors()
            await self._load_daily_stats()
        return new
    ...
    async def activate_live(self, confirmation: str, actor: str = "user") -> dict:
        ...
        self.settings.live.activated_at = datetime.now(timezone.utc).isoformat()
        ...
        self.settings.general.shadow_mode = True  # live starts in shadow mode: one more explicit step to submit orders
        self.settings.trading.auto_execute = False
        self.ctx.settings = self.settings
        if self.repo:
            await self.repo.save_settings_doc(self.settings.model_dump(mode="json"), "app")
            await self.repo.audit(TradingMode.LIVE, "live_activation", "LIVE TRADING ACTIVATED (shadow mode on, auto-execute off)", checklist.as_dict(), actor=actor)
        await self.rebuild_connectors()
        return checklist.as_dict()

    async def deactivate_live(self, actor: str = "user") -> None:
        await self.update_settings({"general": {"mode": TradingMode.PAPER.value}, "live": {"activated": False}}, actor=actor)

    # ------------------------------------------------------------------ exchange accounts
    async def add_exchange_account(self, exchange_id: str, label: str, credentials: dict[str, str], mode: str, sandbox: bool | None = None) -> dict:
        import json
        ...
        acct = ExchangeAccount(id=new_id("exa"), exchange_id=exchange_id, label=label or spec.display_name, mode=mode, enabled=True, sandbox=sandbox, credentials_enc=self.box.encrypt(json.dumps(creds), aad=""), permissions={}, status="supported")
        ...
        self.exchange_accounts = await self.repo.list_exchange_accounts()
        await self.repo.audit(self.mode, "exchange", f"Exchange account added: {spec.display_name} ({mode})", {"exchange": exchange_id, "ok": result["ok"], "permissions": result.get("permissions")})
    ...
    async def _test_credentials(self, exchange_id: str, creds: dict[str, str], sandbox: bool) -> dict:
        c = CcxtConnector(exchange_id, TradingMode.TESTNET if sandbox else TradingMode.LIVE, creds, sandbox=sandbox, timeout_s=self.settings.advanced.connector_timeout_s)
        try:
    ...
    async def test_exchange_account(self, account_id: str) -> dict:
        ...
        await self.repo.update_exchange_account(account_id, status="connected" if result["ok"] else "supported", last_error=None if result["ok"] else result.get("error"), permissions=result.get("permissions", {}), last_verified_at=utcnow() if result["ok"] else acct.last_verified_at)
        self.exchange_accounts = await self.repo.list_exchange_accounts()
        return result
    ...
    async def register_wallet_with_gateway(self, wallet_id: str) -> str | None:
        ...
        try:
            ...
            return None
        if self.repo:
            await self.repo.audit(self.mode, "wallet", f"Bot wallet registered with Gateway ({chain})", {"address": addr})
        return addr
    ...
    async def adjust_paper_funds(self, venue: str, asset: str, amount: Decimal) -> None:
        ...
        await self.repo.save_paper_balances(self.mode, self.ctx.ledger.dump())
        await self._refresh_balances()
        await self.repo.audit(self.mode, "settings_change", f"Paper funds adjusted: {amount} {asset} on {venue}", {})
    ...
    def capital_summary(self) -> dict[str, str]:
        ...
        at_risk = sum((D(t.estimated_costs.get("total", "0")) for t in self.executor.active.values()), ZERO) if self.executor else ZERO
