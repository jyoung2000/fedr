...
export function Money({ value, sign = true, decimals = 2, colored = true, className = "" }: MoneyProps) {
  const s = colored ? signOf(value) : "plain";
  return <span className={`money ${s} ${className}`}>{fmtMoney(value, { sign, decimals })}</span>;
}
