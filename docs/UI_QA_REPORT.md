...
* **`/health` SPA route vs backend `/health` JSON:** client-side navigation to the Health page works and was tested (the mock static server has an SPA fallback), but on the real FastAPI server a hard reload / deep link to `/health` returns the JSON health probe (`main.py` registers `/health` before the SPA catch-all). Needs a backend follow-up (serve `index.html` for `Accept: text/html`, or move the page path); not fixable from `frontend/` alone.
