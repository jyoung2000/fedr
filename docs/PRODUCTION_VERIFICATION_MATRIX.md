...
Generated 2026-09-14 by `scripts/audit_matrix.py` (edit that file, not this one). Version 0.2.0-rc1, git `b456837`.

**production_ready = false.** critical capabilities are not IMPLEMENTED + VERIFIED in this environment: RT-02, RT-03, RT-04, RT-05, RT-06, RT-07, MD-03, CX-01, CX-02, CX-03, CX-04, WL-03, WL-04, MO-02, MO-04, MO-06, ST-02, ST-05.
...
| IMPLEMENTED + PARTIALLY VERIFIED | 16 |
...
Critical rows not yet VERIFIED (18): RT-02 (BLOCKED BY EXTERNAL DEPENDENCY), RT-03 (IMPLEMENTED + PARTIALLY VERIFIED), RT-04 (IMPLEMENTED + PARTIALLY VERIFIED), RT-05 (BLOCKED BY EXTERNAL DEPENDENCY), RT-06 (IMPLEMENTED + PARTIALLY VERIFIED), RT-07 (IMPLEMENTED + PARTIALLY VERIFIED), MD-03 (BLOCKED BY EXTERNAL DEPENDENCY), CX-01 (IMPLEMENTED + PARTIALLY VERIFIED), CX-02 (BLOCKED BY EXTERNAL DEPENDENCY), CX-03 (BLOCKED BY EXTERNAL DEPENDENCY), CX-04 (BLOCKED BY EXTERNAL DEPENDENCY), WL-03 (IMPLEMENTED + PARTIALLY VERIFIED), WL-04 (IMPLEMENTED + PARTIALLY VERIFIED), MO-02 (BLOCKED BY EXTERNAL DEPENDENCY), MO-04 (BLOCKED BY EXTERNAL DEPENDENCY), MO-06 (MUST BLOCK), ST-02 (IMPLEMENTED + PARTIALLY VERIFIED), ST-05 (IMPLEMENTED + PARTIALLY VERIFIED)
...
| RT-03 | Image build + container runtime with substituted (locally imported) base images: pip install, npm ci, vite build, non-root user, healthcheck | **IMPLEMENTED + PARTIALLY VERIFIED** | docker/Dockerfile built with --build-arg BASE_IMAGE=fedr/base-local:sandbox (a trimmed copy of this machine's Debian rootfs). See docs/DOCKER_SECURITY_REPORT.md for the exact commands and outputs. | Substitute base images are not the images users get; treat as a topology/runtime test, not a build reproducibility test. | yes |
| RT-04 | Compose stack up, /health, restart, down/up persistence, gateway not published | **IMPLEMENTED + PARTIALLY VERIFIED** | docs/DOCKER_SECURITY_REPORT.md (runtime section). The gateway service could not run (RT-05); the app service was exercised with `docker compose up -d --no-deps app` plus a stand-in container on the internal network to prove non-exposure. | Repeat with the real Gateway image on a host with registry access. | yes |
...
| RT-06 | Gateway is never published on a host port; app publishes 127.0.0.1:8935 only | **IMPLEMENTED + PARTIALLY VERIFIED** | `docker compose config` shows no `ports:` on the gateway service and `127.0.0.1:8935:8935` on app; runtime check with a stand-in gateway container in docs/DOCKER_SECURITY_REPORT.md. | Runtime check used a stand-in container, not the Gateway image. | yes |
| RT-07 | Container hardening: non-root uid 10001, cap_drop ALL, no-new-privileges, read-only root FS, tmpfs /tmp, /data volume | **IMPLEMENTED + PARTIALLY VERIFIED** | Static: docker-compose.yml, docker/Dockerfile. Runtime: docs/DOCKER_SECURITY_REPORT.md (id, capabilities, read-only test inside the running container). | Verified in the sandbox daemon (overlay2, no iptables); re-run on the target host. | yes |
...
| RT-10 | Startup fails safely on unsafe/missing critical configuration (live boot default, live-allowed without token, bad master key, unwritable data dir) | **IMPLEMENTED + VERIFIED** | fedr/config/env.py::validate_startup; tests/test_api.py startup tests; process exits non-zero with the reason. | — | yes |
...
- **RT-03** Image build + container runtime with substituted (locally imported) base images: pip install, npm ci, vite build, non-root user, healthcheck — IMPLEMENTED + PARTIALLY VERIFIED: Substitute base images are not the images users get; treat as a topology/runtime test, not a build reproducibility test.
- **RT-04** Compose stack up, /health, restart, down/up persistence, gateway not published — IMPLEMENTED + PARTIALLY VERIFIED: Repeat with the real Gateway image on a host with registry access.
...
- **RT-06** Gateway is never published on a host port; app publishes 127.0.0.1:8935 only — IMPLEMENTED + PARTIALLY VERIFIED: Runtime check used a stand-in container, not the Gateway image.
- **RT-07** Container hardening: non-root uid 10001, cap_drop ALL, no-new-privileges, read-only root FS, tmpfs /tmp, /data volume — IMPLEMENTED + PARTIALLY VERIFIED: Verified in the sandbox daemon (overlay2, no iptables); re-run on the target host.
